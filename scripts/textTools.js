var texttools = {
    ...
    kolor: {
        pattern: "[color=%p]%s[/color]",
        accesskey: "c",
        prompt: "Wpisz nazwę koloru (red, #ff6, #ccff33, itp.)",
    },
    url: {
        pattern: "[url=%p]%s[/url]",
        accesskey: "l",
        prompt: "Wpisz adres URL (lub pozostaw puste jeśli go zaznaczyłeś)",
        default: "http://",
    },
    cytat: {
        pattern: "[quote=%p]%s[/quote]",
        accesskey: "c",
        prompt: "Wpisz nick osoby, której tekst cytujesz (opcjonalne)",
    },
    ...
    if (textarea[ita].getAttribute("name") == "req_message") {
            ...
            textarea.addEventListener(
                ...
                function (e) {
                    if (
                        e.altKey &&
                        e.ctrlKey &&
                        (butt = operaAccessKeyHack[e.keyCode])
                    ) {
                        submittedButt = butt;
                        butt.click();
                    }
                ...
                tt.appendChild(
                    createBBCodeButt(
                        ...
                        function (e) {
                            ...
                            var nsel = texttools[tool]["pattern"].replace(
                                /%s/g,
                                osel[0]
                            );
                            var p = null;
                            if (
                                texttools[tool]["pattern"].indexOf("=%p") != -1
                            ) {
                                p = prompt(
                                    texttools[tool]["prompt"]
                                        ? texttools[tool]["prompt"]
                                        : "Type parameter (%p) for \u201c" +
                                              nsel +
                                              "\u201d\u00a0:",
                                    texttools[tool]["default"]
                                        ? texttools[tool]["default"]
                                        : ""
                                );
                            ...
                            textarea.value =
                                textarea.value.substr(0, osel[1]) +
                                nsel +
                                textarea.value.substr(osel[2]);
                            if (osel[1] != osel[2]) {
                                textarea.selectionStart = osel[1];
                                textarea.selectionEnd = osel[1] + nsel.length;
                            } else {
                                textarea.selectionStart =
                                    osel[1] + nsel.indexOf("[", 1);
                                textarea.selectionEnd = textarea.selectionStart;
                            }
                        },
                        texttools[tool]["pattern"],
                        texttools[tool]["accesskey"]
                    )
                );
        ...
        if (form != null) {
            ...
            if (tid) {
                ...
                var preview = stuff.appendChild(
                    createInput("submit", "preview", "Preview")
                );
                ...
                lbl.appendChild(
                    document.createTextNode("Subskrybuj ten temat")
                );
            ...
            for (var sb = 0; sb < subs.length; sb++) {
                /*opens in new window because of shift:/*/
                if (
                    subs[sb].getAttribute("type") == "submit" &&
                    (accesskey = subs[sb].getAttribute("accesskey"))
                ) {
                    operaAccessKeyHack[accesskey.toLowerCase().charCodeAt(0)] =
                        subs[sb];
...
function createBBCodeButt(text, link, title, accesskey) {
    ...
    if (accesskey) {
        ...
        a.appendChild(document.createElement("strong")).appendChild(
            document.createTextNode(text.charAt(kpos))
        );
